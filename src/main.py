...
class App:
    ...
    def start_main_ui(self):
        ...
        try:
            ...
            self.log(f"FEHLER Hotkeys: {e} (Als Admin starten!)")
    ...
    def scan_once(self):
        ...
        if self.template_tl is None:
            self.log("⚠️ Keine Templates geladen. Bitte F10 drücken.")
            return
        
        self.is_scanning = True
        self.progress_read["value"] = 0
        self.lbl_progress.config(text="Suche Fenster...")
    ...
    def load_cached_templates(self):
        ...
        if os.path.exists(p_tl) and os.path.exists(p_br):
            try:
                ...
                self.log("✅ Templates geladen. Bereit für F9.")
            except: 
                self.log("⚠️ Fehler beim Laden der Templates.")
        else:
            self.log("ℹ️ Keine gespeicherten Templates. Bitte F10 drücken.")
    ...
    def _step2_finished(self, x, y, w, h):
        ...
        if found:
            ...
            self.log("⚠️ Fehler: Konnte das Fenster mit diesen Ecken nicht finden.")
